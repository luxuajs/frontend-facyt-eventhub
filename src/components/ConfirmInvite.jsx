...
export default function ConfirmInvite({ token, onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Error al activar tu cuenta. El enlace podría haber expirado.');
      }

      setMessage(data.message || 'Cuenta de coordinador configurada con éxito.');
    } catch (err) {
      ...
      <div className="max-w-md w-full space-y-8 p-8 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md rounded-2xl shadow-xl transition-all duration-150">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-4 text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
            Activar Cuenta de Coordinador
          </h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Configura tu contraseña de acceso para comenzar a coordinar espacios físicos
          </p>
        </div>
        ...
        {!message ? (
          <form className="mt-8 space-y-4" onSubmit={handleSubmit} noValidate>
            ...
            {/* Password */}
            <div>
              <label htmlFor="invite-pass" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                Nueva Contraseña
              </label>
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="invite-pass-confirm" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
            ...
            <div className="pt-2">
              ...
              >
                {loading ? 'Activando...' : 'Establecer Contraseña'}
              </button>
            </div>
          </form>
        ) : (
